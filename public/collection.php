...
$app->get('/get', function(Request $request) use($app)
{
    ...
    if(!$conn->connect_error) {
        $query = "SELECT c.* , u.name as users_name, a.name as agent_name, w.name as waste_name, co.name as coverage_name
FROM collection c
JOIN users u ON u.users_id = c.users_id
JOIN coverage co ON co.coverage_id = u.coverage_id
